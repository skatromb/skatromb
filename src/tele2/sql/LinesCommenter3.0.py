...
# Где искать
FILE_PATHS = (r"/Users/python_skatromb/PycharmProjects/Python/src/allMyCats.py",)

# Что искать
SUBSTRS_TO_MATCH = ("print",)
...
def remove_lines(
    ...
    substrs_to_match: tuple[str],
    lines_will_be="commented",
    ...
    def match(code_line: str) -> bool:
        ...
        line_matches_what_we_search = True in [
            match_line_with_each(substr) for substr in substrs_to_match
        ]
    ...
    def modify(code_line: str) -> str:
        ...
        if lines_will_be == "commented":
            ...
            raise Exception(
                "lines_will_be должен быть из " + str(WHAT_TO_DO_DICT.keys())
            )
        return new_line
    ...
    def get_modified_version(file: TextIO) -> list[str]:
        ...
        for line_number, code_line in enumerate(file):
            if match(code_line):
                lines_for_print.append(
                    {"number": str(line_number + 1), "src": code_line}
                )
                modified_lines.append(modify(code_line))
            else:
                modified_lines.append(code_line)
            just = (
                len(str(line_number + 1)) + 1
            )  # Вычисляем отступ для красивого форматирования вывода строк
        ...
        for line_for_print in lines_for_print:
            print(
                line_for_print["number"].rjust(just) + ": " + line_for_print["src"],
                end="",
            )
    ...
    if input(
        WHAT_TO_DO_DICT[lines_will_be] + " эти строки? (y/yes, n/no)\n"
    ).lower() not in ("y", "yes"):
        exit()
    else:
        for file in files:
...
remove_lines(
    FILE_PATHS, SUBSTRS_TO_MATCH, lines_will_be="deleted", ignore_commented=False
)
