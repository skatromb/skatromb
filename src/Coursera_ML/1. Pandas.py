...
print(
    "3: \n"
    "{:.2f}".format(
        float(data["Pclass"].value_counts()[1] / data.shape[0]).__round__(4) * 100
    ),
    end="\n" * 3,
)

print("4: \n" "{:.2f}".format(float(data["Age"].mean()).__round__(4)), end="\n" * 3)
...
print(
    "5: \n"
    "{:.2f}".format(
        float(data["SibSp"].corr(other=data["Parch"], method="pearson")).__round__(4)
    ),
    end="\n" * 3,
)
