...
def fill_template(
    ...
    name_tags = tree.findall(
        name_search_pattern, {"NAME_TAG": "http://www.w3.org/2000/svg"}
    )
    ...
    if len(name_tags) == 1:
        ...
        raise Exception(
            "В шаблоне svg оказалось больше одного места для подстановки ФИ"
        )
...
def template2png(
    ...
    cmd = [
        f'"{chrome_params["google_chrome_path"]}" {chrome_params["screenshot_params"]}='
