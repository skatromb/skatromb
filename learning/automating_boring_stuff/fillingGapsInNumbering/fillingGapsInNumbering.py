...
def find_and_parse_files_with_prefix(
    prefix: str, folder="."
) -> list[ParsedFilenameDict]:
                ...
                parsed_filenames.append(
                    {
                        ...
                        "numbering": matches.group(NUMBERING),
                        "postfix": matches.group(POSTFIX),
                    }
                )
...
def get_new_names(
    ...
    longest_num_len = len(
        max(parsed_filenames, key=lambda filename: len(filename["numbering"]))[
            "numbering"
        ]
    )

    # Map a right numbering for the files
    for parsed_filename, i in zip(
        parsed_filenames, range(1, len(parsed_filenames) + 1)
    ):
        new_numbering_str = str(i).rjust(longest_num_len, "0")
        new_filename = (
            parsed_filename["prefix"]
            + new_numbering_str
            + parsed_filename["postfix"]
        )
        old_new_filenames.append(
            {
                "old_filename": parsed_filename["filename"],
...
# Rename itself!
def rename_files(
    old_new_filenames: list[OldNewFilename], folder="."
) -> list[OldNewFilename]:
...
#  Main program
parsed_filenames_list = find_and_parse_files_with_prefix(
    prefix="spam", folder="."
)
